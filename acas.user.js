// ==UserScript==
// @name        ACAS-bot (Advanced Chess Assistance System)
...
function getRights() {
  ...
  if (useChess960) {
    ...
    const e1 = getBoardPiece("e1"), h1 = getBoardPiece("h1"), a1 = getBoardPiece("a1");
    if (e1 == "K" && h1 == "R")
      rights += "K";
    if (e1 == "K" && a1 == "R")
      rights += "Q";
    const e8 = getBoardPiece("e8"), h8 = getBoardPiece("h8"), a8 = getBoardPiece("a8");
    if (e8 == "k" && h8 == "r")
      rights += "k";
    if (e8 == "k" && a8 == "r")
      rights += "q";
...
function getBoardSquareChangeAmount(lastFen, newFen) {
  ...
  for (let i = 0;i < board1.length; i++) {
    if (board1[i] !== board2[i]) {
      if (board1[i]?.trim()?.length > 0)
        changedFrom.push(board1[i]?.toLowerCase());
      diff += 1;
    }
  }
  return diff;
    ...
    if (elementToTrigger) {
      switch (domain) {
        case "chess.com":
          ...
          if (this.isLegit)
            await this.delay(this.getRandomIntegerNearAverage(35, 125));
...
var activeGuiMoveMarkings = [];
var activeMetricRenders = [];
var activeFeedback = [];
